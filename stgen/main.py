...
def info(args):
    ...
    print(
        "STGen is part of the Coffe benchmark and enables stressful test case generation."
    )
    ...
    if args.level == "func":
        if os.path.isdir(args.output_path):
            output_file = os.path.join(
                args.output_path, "func_stressful_testcases.json"
            )
        else:
            output_file = args.output_path
        gen_func_sts(
        ...
        if not args.solution_file:
            raise ValueError(
                "Path to solution file must be indicated with option -s when generating file-level stressful test cases!"
            )
        if os.path.isdir(args.output_path):
            output_file = os.path.join(
                args.output_path, "file_stressful_testcases.json"
            )
        else:
            output_file = args.output_path
        gen_file_sts(
...
def pipe(args):
    ...
    if args.level == "func":
        ...
        if os.path.isdir(args.output_path):
            ...
            contract_file = os.path.join(
                os.path.dirname(args.output_path), "file_contracts.json"
            )
            st_file = args.output_path
        print(colored("+++++++++++Step 1: Generating Contracts..", "green"))
        gen_file_contracts(
...
def main():
    ...
    contract_parser.add_argument(
        ...
        required=True,
        type=str,
        help="Path to the output contract file",
    )
    contract_parser.add_argument(
        "-l",
        "--level",
        ...
        default="func",
        type=str,
        help="Should be func for function-level or file for file-level stressful test cases.",
    )
    contract_parser.add_argument(
        "-d", "--data_file", required=True, type=str, help="Path to the data file"
    )
    contract_parser.add_argument(
        "-t",
        "--test_file",
        required=True,
        type=str,
        help="Path to the correctness test case file",
    )
    contract_parser.add_argument(
        "-s",
        "--solution_file",
        required=True,
        type=str,
        help="Path to the ground truth solution file",
    )
    contract_parser.add_argument(
        "-v",
        "--verbose",
    ...
    st_parser.add_argument(
        ...
        type=int,
        default=5,
        help="Number of stressful test cases to generate for each instance",
    )
    st_parser.add_argument(
        "-v",
        "--verbose",
